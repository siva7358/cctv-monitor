...
# Complete camera data - JB-1 to JB-180
camera_data = [
    {
        "jb": "JB-1",
...
def get_status():
    """Return status of all cameras with location details"""
    results = []
    ...
    for device in camera_data:
        ...
        if online_count == len(device['ips']):
            overall = 'online'
        elif online_count > 0:
            overall = 'partial'
        else:
        ...
        results.append({
            ...
            'totalCount': len(device['ips']),
            'overallStatus': overall
        })
    
    return jsonify(results)

@app.route('/api/status/<jb_name>')
def get_jb_status(jb_name):
    """Return status for a specific JB"""
    for device in camera_data:
        if device['jb'] == jb_name:
            ip_statuses = []
            online_count = 0
            ...
            return jsonify({
                ...
                'onlineCount': online_count,
                'totalCount': len(device['ips']),
                'overallStatus': 'online' if online_count == len(device['ips']) else 'partial' if online_count > 0 else 'offline'
            })
    
    return jsonify({'error': 'JB not found'}), 404

@app.route('/api/all-ips')
def get_all_ips():
    """Return all IPs grouped by JB"""
    all_ips = {}
    for device in camera_data:
        all_ips[device['jb']] = {
            'ips': device['ips'],
...
def get_locations():
    """Return all locations with coordinates"""
    locations = []
    for device in camera_data:
        locations.append({
            'jb': device['jb'],
...
def get_online_count():
    """Return count of online cameras per JB"""
    results = []
    for device in camera_data:
        online_count = 0
        for ip in device['ips']:
...
if __name__ == '__main__':
    print(f"🚀 Camera Monitoring API Started!")
    print(f"📊 Loaded {len(camera_data)} JB devices")
    print(f"📍 Total locations: {len(camera_data)}")
